...
/// @param roll in rad
/// @return
inline tam::types::common::Vector3D<double> angular_velocities_to_2d(
  const tam::types::common::Vector3D<double> & angular_velocities, const double pitch,
  const double roll)
{
  typedef tam::types::common::EulerYPR euler_t;
  euler_t out = math::rotate_angular_velocities_to_2D(
    euler_t(angular_velocities.z, angular_velocities.y, angular_velocities.x),
    euler_t(0.0, pitch, roll));
  return tam::types::common::Vector3D<double>(out.roll, out.pitch, out.yaw);
}
}  // namespace tam::helpers::euler_rotations
