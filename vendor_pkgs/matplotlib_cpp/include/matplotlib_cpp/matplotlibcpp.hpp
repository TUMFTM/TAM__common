...
private:
  ...
  _interpreter()
  {
    ...
    if (
      ...
      !s_python_function_pause || !s_python_function_figure || !s_python_function_fignum_exists ||
      !s_python_function_plot || !s_python_function_quiver || !s_python_function_contour ||
      !s_python_function_colorbar || !s_python_function_semilogx || !s_python_function_semilogy ||
      !s_python_function_loglog || !s_python_function_fill || !s_python_function_fill_between ||
      !s_python_function_subplot || !s_python_function_legend || !s_python_function_ylim ||
      !s_python_function_title || !s_python_function_axis || !s_python_function_xlabel ||
      !s_python_function_ylabel || !s_python_function_xticks || !s_python_function_yticks ||
      !s_python_function_xscale || !s_python_function_yscale || !s_python_function_grid ||
      !s_python_function_xlim || !s_python_function_ion || !s_python_function_ginput ||
      !s_python_function_save || !s_python_function_clf || !s_python_function_annotate ||
      !s_python_function_errorbar || !s_python_function_errorbar ||
      !s_python_function_tight_layout || !s_python_function_stem || !s_python_function_xkcd ||
      !s_python_function_text || !s_python_function_suptitle || !s_python_function_bar ||
      !s_python_function_subplots_adjust || !s_python_function_spy || !s_python_function_imshow) {
      throw std::runtime_error("Couldn't find required function!");
    }

    if (
      !PyFunction_Check(s_python_function_show) || !PyFunction_Check(s_python_function_close) ||
      !PyFunction_Check(s_python_function_draw) || !PyFunction_Check(s_python_function_pause) ||
      !PyFunction_Check(s_python_function_figure) ||
      !PyFunction_Check(s_python_function_fignum_exists) ||
      !PyFunction_Check(s_python_function_plot) || !PyFunction_Check(s_python_function_quiver) ||
      ...
      !PyFunction_Check(s_python_function_loglog) || !PyFunction_Check(s_python_function_fill) ||
      !PyFunction_Check(s_python_function_fill_between) ||
      !PyFunction_Check(s_python_function_spy) || !PyFunction_Check(s_python_function_subplot) ||
      !PyFunction_Check(s_python_function_legend) ||
      !PyFunction_Check(s_python_function_annotate) || !PyFunction_Check(s_python_function_ylim) ||
      ...
      !PyFunction_Check(s_python_function_xticks) || !PyFunction_Check(s_python_function_yticks) ||
      !PyFunction_Check(s_python_function_xscale) || !PyFunction_Check(s_python_function_yscale) ||
      !PyFunction_Check(s_python_function_grid) || !PyFunction_Check(s_python_function_xlim) ||
      !PyFunction_Check(s_python_function_ion) || !PyFunction_Check(s_python_function_ginput) ||
      !PyFunction_Check(s_python_function_save) || !PyFunction_Check(s_python_function_clf) ||
      !PyFunction_Check(s_python_function_tight_layout) ||
      !PyFunction_Check(s_python_function_errorbar) || !PyFunction_Check(s_python_function_stem) ||
      !PyFunction_Check(s_python_function_xkcd) || !PyFunction_Check(s_python_function_text) ||
      !PyFunction_Check(s_python_function_suptitle) || !PyFunction_Check(s_python_function_bar) ||
...
PyObject * get_2darray(const std::vector<::std::vector<Numeric>> & v)
{
  detail::_interpreter::get();  // interpreter needs to be initialized for the
                                // numpy commands to work
  if (v.size() < 1) throw std::runtime_error("get_2darray v too small");
...
PyObject * get_2darray(const Matrix & A)
{
  detail::_interpreter::get();  // interpreter needs to be initialized for the
                                // numpy commands to work
  if (A.size() < 1) throw std::runtime_error("get_2darray A too small");

  npy_intp vsize[2] = {static_cast<npy_intp>(A.rows()), static_cast<npy_intp>(A.cols())};
...
bool plot_base(
  PyObject * const pyfunc, const VectorX & x, const VectorY & y, const std::string & s = "",
  const std::map<std::string, std::string> & keywords = {})
{
  ...
  for (auto const & item : keywords) {
    PyDict_SetItemString(kwargs, item.first.c_str(), PyString_FromString(item.second.c_str()));
  }
...
template <typename VectorX, typename VectorY>
bool plot(
  const VectorX & x, const VectorY & y, const std::string & s = "",
  const std::map<std::string, std::string> & keywords = {})
{
  return detail::plot_base(detail::_interpreter::get().s_python_function_plot, x, y, s, keywords);
}
// @brief standard plot function without formatting string, needed if
//        keywords are given but formatting string is not
template <typename VectorX, typename VectorY>
bool plot(const VectorX & x, const VectorY & y, const std::map<std::string, std::string> & keywords)
{
...
bool plot(const VectorY & y, const std::string & format = "")
{
  ...
  for (long unsigned int i = 0; i < x.size(); ++i) x.at(i) = i;
...
//        string is missing
template <typename VectorY = std::vector<double>>
bool plot(const VectorY & y, const std::map<std::string, std::string> & keywords)
{
  std::vector<int> x(y.size());
  for (int i = 0; i < x.size(); ++i) x.at(i) = i;
...
bool loglog(
  ...
  const std::map<std::string, std::string> & keywords = {})
{
  return detail::plot_base(detail::_interpreter::get().s_python_function_loglog, x, y, s, keywords);
}
template <typename VectorX, typename VectorY>
bool loglog(
  const VectorX & x, const VectorY & y, const std::map<std::string, std::string> & keywords)
{
  return loglog(x, y, "", keywords);
}
template <typename VectorY = std::vector<double>>
bool loglog(
  const VectorY & y, const std::string & s = "",
  const std::map<std::string, std::string> & keywords = {})
{
...
template <typename VectorY = std::vector<double>>
bool loglog(const VectorY & y, const std::map<std::string, std::string> & keywords)
{
  std::vector<std::size_t> x(y.size());
  for (std::size_t i = 0; i < x.size(); ++i) x.at(i) = i;
  ...
  const std::map<std::string, std::string> & keywords = {})
{
  return detail::plot_base(
    detail::_interpreter::get().s_python_function_semilogx, x, y, s, keywords);
}
template <typename VectorX, typename VectorY>
bool semilogx(
  const VectorX & x, const VectorY & y, const std::map<std::string, std::string> & keywords)
...
template <typename VectorY = std::vector<double>>
bool semilogx(const VectorY & y, const std::map<std::string, std::string> & keywords)
{
  std::vector<std::size_t> x(y.size());
  for (std::size_t i = 0; i < x.size(); ++i) x.at(i) = i;
...
{
  return detail::plot_base(
    detail::_interpreter::get().s_python_function_semilogy, x, y, s, keywords);
}
template <typename VectorX, typename VectorY>
bool semilogy(
  const VectorX & x, const VectorY & y, const std::map<std::string, std::string> & keywords)
...
template <typename VectorY = std::vector<double>>
bool semilogy(const VectorY & y, const std::map<std::string, std::string> & keywords)
{
  std::vector<std::size_t> x(y.size());
  for (std::size_t i = 0; i < x.size(); ++i) x.at(i) = i;
...
void imshow(const Matrix & X, const std::map<std::string, std::string> & keywords = {})
{
  ...
  PyObject * res =
    PyObject_Call(detail::_interpreter::get().s_python_function_imshow, plot_args, kwargs);
...
template <typename Matrix>
void plot_surface(
  const Matrix & x, const Matrix & y, const Matrix & z,
...
template <typename Numeric>
bool fill_between(
  const std::vector<Numeric> & x, const std::vector<Numeric> & y1, const std::vector<Numeric> & y2,
  const std::map<std::string, std::string> & keywords)
{
...
bool hist(
  const VectorY & y, long bins = 10, std::string color = "b", double alpha = 1.0,
...
template <typename VectorX, typename VectorY>
bool scatter(
  const VectorX & x, const VectorY & y, const double s = 1.0,
  const std::map<std::string, std::string> keywords = {})
{
  assert(x.size() == y.size());
...
// @param keywords Additional keywords
template <typename Matrix>
bool spy(const Matrix & A, const std::map<std::string, std::string> & keywords = {})
{
...
template <typename Numeric>
bool bar(
  const std::vector<Numeric> & y, std::string ec = "black", std::string ls = "-", double lw = 1.0,
  const std::map<std::string, std::string> & keywords = {})
{
  PyObject * yarray = get_array(y);
...
template <typename Numeric>
bool named_hist(
  std::string label, const std::vector<Numeric> & y, long bins = 10, std::string color = "b",
  double alpha = 1.0)
{
  PyObject * yarray = get_array(y);
...
bool quiver(
  const std::vector<NumericX> & x, const std::vector<NumericY> & y, const std::vector<NumericU> & u,
...
void axhline(const NumericY y, const std::map<std::string, std::string> keywords = {})
{
  detail::_interpreter::get();
  ...
  PyObject * res = PyObject_Call(
    detail::_interpreter::get().s_python_function_axhline,
    detail::_interpreter::get().s_python_empty_tuple, kwargs);

  Py_DECREF(kwargs);

  if (!res) throw std::runtime_error("Call to axhline() failed.");

  Py_DECREF(res);
}
template <typename NumericX>
void axvline(const NumericX x, const std::map<std::string, std::string> keywords = {})
{
  detail::_interpreter::get();
  ...
  PyObject * res = PyObject_Call(
    detail::_interpreter::get().s_python_function_axvline,
    detail::_interpreter::get().s_python_empty_tuple, kwargs);
...
bool stem(
  const std::vector<NumericX> & x, const std::vector<NumericY> & y, const std::string & s = "")
{
  assert(x.size() == y.size());
...
template <typename VectorX, typename VectorY>
bool errorbar(
  const VectorX & x, const VectorY & y, const VectorY & yerr,
  const std::map<std::string, std::string> & keywords = {})
{
  assert(x.size() == y.size());
...
inline void legend(
  const std::string & loc = "best", const Vector & bbox_to_anchor = Vector(),
  const std::map<std::string, std::string> & keywords = {})
{
  detail::_interpreter::get();
  ...
  // add location
  if (loc != "") PyDict_SetItemString(kwargs, "loc", PyString_FromString(loc.c_str()));
...
template <typename Vector>
inline void legend(
  const Vector & bbox_to_anchor, const std::map<std::string, std::string> & keywords = {})
{
  legend("", bbox_to_anchor, keywords);
}
inline void legend(
  const std::string & loc, const std::map<std::string, std::string> & keywords = {})
{
...
inline void legend(const char loc[], const std::map<std::string, std::string> & keywords = {})
{
  legend(loc, std::vector<double>(), keywords);
}
/*
...
template <typename Numeric>
inline void xticks(
  const std::vector<Numeric> & ticks, const std::vector<std::string> & labels = {},
  const std::map<std::string, std::string> & keywords = {})
{
...
template <typename Numeric>
inline void xticks(
  const std::vector<Numeric> & ticks, const std::map<std::string, std::string> & keywords)
{
...
inline void title(
  const std::string & titlestr, const std::map<std::string, std::string> & keywords = {})
{
  detail::_interpreter::get();
...
inline void suptitle(
  const std::string & suptitlestr, const std::map<std::string, std::string> & keywords = {})
{
  detail::_interpreter::get();
...
inline void xlabel(
  const std::string & str, const std::map<std::string, std::string> & keywords = {})
{
...
inline void savefig(
  const std::string & filename, const std::map<std::string, std::string> & keywords = {})
{
