...
{
  // TODO(Simon S) Check which coordinate system this is
  ...
  ControlConstraintPointAsPolygon operator+(const ControlConstraintPointAsPolygon & other) const
  {
    ControlConstraintPointAsPolygon result = *this;

    if (a_lim.size() == other.a_lim.size()) {
      for (size_t i = 0; i < a_lim.size(); ++i) {
        result.a_lim[i] = a_lim[i] + other.a_lim[i];
      }
    } else {
      // Handle size mismatch if necessary
      std::cerr << "[Types: Adding ControlConstraintPointAsPolygon] Vector sizes do not match!"
                << std::endl;
    }

    result.lateral_error_min_m += other.lateral_error_min_m;
    result.lateral_error_max_m += other.lateral_error_max_m;
  ...
  ControlConstraintPointAsPolygon operator-(const ControlConstraintPointAsPolygon & other) const
  {
    ...
    if (a_lim.size() == other.a_lim.size()) {
      ...
      // Handle size mismatch if necessary
      std::cerr << "[Types: Subtracting ControlConstraintPointAsPolygon] Vector sizes do not match!"
                << std::endl;
    }
  ...
  ControlConstraintPointAsPolygon operator*(double scalar) const
  {
    ControlConstraintPointAsPolygon result = *this;

    for (size_t i = 0; i < a_lim.size(); ++i) {
      result.a_lim[i] = a_lim[i] * scalar;
    }

    result.lateral_error_min_m *= scalar;
    result.lateral_error_max_m *= scalar;

    return result;
  }
  // Friend operator for multiplying a scalar (double) by a ControlConstraintPointAsPolygon instance
  friend ControlConstraintPointAsPolygon operator*(
    double scalar, const ControlConstraintPointAsPolygon & polygon)
  {
...
// Previously we also had // This will be removed

// FIXME(Simon S) REMOVE THIS SHIT
...
{
  tam::types::common::Vector2D<double> a_lim_mps2[12];
};
constexpr size_t mpc_horizon_length = 36;
...
struct acados_mpc_debug
{
  ...
  double banking_traj_rad[mpc_horizon_length + 1];
  double slope_traj_rad[mpc_horizon_length + 1];
  ...
  double tartraj_slope_rad[planner_traj_length];
  TUMAccLimitDebug tartraj_a_lim_mps2[planner_traj_length];

  // resampled for controller
  int8_t number_a_lim_points;
  TUMAccLimitDebug a_lim_mps2[mpc_horizon_length + 1];
