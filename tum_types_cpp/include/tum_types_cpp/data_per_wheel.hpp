...
#undef OVERLOAD_OPERATOR_COMPONENT_WISE_COMPARISON
  ...
  constexpr std::string_view name_at(std::size_t i) const
  {
    switch (i) {
      // clang-format off
      case static_cast<std::size_t>(SignalOrder::front_left): return "front_left";
...
// logger type suppor via overload
namespace tam::tsl::type_support
{
template <typename T>
inline void log(
  TypeSupportInterface * logger, std::string const & name,
  tam::types::common::DataPerWheel<T> const & value)
{
  // Log data per wheel type
  log<T>(logger, name + "/front_left", value.front_left);
  log<T>(logger, name + "/front_right", value.front_right);
  log<T>(logger, name + "/rear_left", value.rear_left);
  log<T>(logger, name + "/rear_right", value.rear_right);
}
}  // namespace tam::tsl::type_support